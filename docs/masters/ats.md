...
Principal Software Engineer at Optum, the technology arm of UnitedHealth Group, building enterprise-grade platform infrastructure since 2019. Deep backend and distributed systems owner: custom Kubernetes Operators in Go, Kafka-as-a-Service and Elasticsearch-as-a-Service control planes, GKE environment orchestration, and Terraform-driven infrastructure as code across on-premise and multi-cloud environments. Leads architecture design, multi-year roadmaps, and platform management for systems that move 20+ petabytes of data and serve hundreds of enterprise teams, with hands-on SRE ownership of production streaming infrastructure spanning 800+ clusters, thousands of nodes, and ~2,000 GCP compute instances at five nines reliability with zero data loss. Partners with VP and SVP-level leaders and with vendor partners (Warpstream, GCP) to shape platform direction, and builds production AI agents and agent skills with responsible-use review gates designed into the architecture.
...
- **Multi-year Kafka platform roadmap:** Led the multi-year roadmap that took the Kafka service from CI/CD pipeline provisioning to web-interface management in the enterprise product marketplace.
- **Schema registry migration roadmap:** Led the multi-quarter roadmap migrating from a self-hosted schema registry to Confluent Schema Registry.
- **Executive influence:** Developed a Kubernetes security report for a VP to shape the organization's use of Kubernetes on GCP; partner with an SVP-level leader to build an org-wide SRE team.
...
- **Automated deployment pipeline initiative:** Lead the fully automated deployment pipeline initiative covering code and image development, vulnerability remediation, and Kubernetes deployment at scale across **30 GKE clusters**.
- **GCP vendor partnership and capacity management:** Run approximately **2,000 GCP compute instances**; maintain the working relationship with GCP partners, managing committed use discount (CUD) allocation, compute reservations, and resource right-sizing, influencing GCP feature requests, and gathering and implementing their recommendations for the organization's usage.
...
- **On-call and support agent skills:** Built agent skills that encode playbooked on-call and chat support actions; in use by the engineering teams and distributed across the organization for general use and as a base for other teams building their own skills.
- **AI support agents:** Building AI agents that handle the team's day-to-day chat support, saving **80 hours per week** of engineering time (**4,160 hours per year**); integrated with ServiceNow and customer documentation, with read-only access to Kubernetes control planes and Thanos data sources for full-context debugging.
- **Responsible AI practices:** Build responsible use into the agent architecture through extensive testing and review gates, and reduce hallucination with purpose-built agent tools that make runs deterministic; completed training in safe AI agent development and usage.
...
- Lead the Warpstream and Optum vendor partnership for the Warpstream on GCP offering, and lead the roadmap for Warpstream Clusters on GCP as a net-new product.
